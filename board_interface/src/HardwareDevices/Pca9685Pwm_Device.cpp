 ...
 * @author
 */
class Pca9685Pwm_Device : public Device {
private:
...
const static uint8_t deviceTypeId = HardwareDescriptor::DEVICE_PWM; // The type ID for this device

// based on HD
//
inline bool pinModeIsValid(PinMode mode) {
	return (mode == PWM_ON_MODE || mode == PWM_OFF_MODE);
} // pinModeIsGpioMode

//
inline bool pinStateIsValid(PinState state) {
	return (state == VARIABLE_STATE || state == NO_STATE);
} // pinModeIsGpioMode
// ---------------------------------------------------------------------------
// ***************************************************************************
